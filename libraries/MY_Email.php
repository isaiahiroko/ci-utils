...
{
    // DB table
    private $table_email_queue = 'email_queues';

    // Main controller
    private $main_controller = 'sys/email_queue/send_pending_emails';
    ...
    public function send($skip_job = FALSE)
    {
        ...
        $dbdata = array(
            'email_queue_id' =>  Uuid::uuid4()->toString(),
            'to' => $to,
            'cc' => $cc,
            'bcc' => $bcc,
            'message' => $this->_body,
    ...
    public function send_queue()
    {
        ...
        foreach ($emails as $email)
        {
            ...
            if ($this->send(TRUE)) {
                ...
                log_message('error', parent::print_debugger());
            } else {
                $status = 'failed';
            ...
            log_message('error', '[Email] out');
            log_message('error', parent::print_debugger());
            // die(var_dump($recipients,$email->email_queue_id, $status));

            $this->CI->db->where('email_queue_id', $email->email_queue_id);
